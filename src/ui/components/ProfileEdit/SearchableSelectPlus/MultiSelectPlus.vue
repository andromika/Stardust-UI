...
import './MultiSelectPlus.scss';

defineOptions({ inheritAttrs: false });

export type { SearchableSelectPlusOption as MultiSelectPlusOption };

const props = withDefaults(
  defineProps<{
    modelValue?: (string | number)[];
    options?: SearchableSelectPlusOption[];
    /** Get value from option (default: opt.value) */
    valueKey?: string;
    /** Get label from option (default: opt.label) */
    labelKey?: string;
    /** Custom filter: (options, searchQuery) => filtered options */
    filter?: (opts: SearchableSelectPlusOption[], query: string) => SearchableSelectPlusOption[];
    placeholder?: string;
    ...
    columns?: number;
    /** Close menu when clicking outside; set false to keep open until explicit close. */
    closeOnOutsideClick?: boolean;
    triggerAriaLabel?: string;
    /** Max number of tags visible in trigger before showing "+N" overflow */
...
function getOptionLabel(opt: SearchableSelectPlusOption): string {
...
const selectedOptions = computed(() =>
  currentValues.value
    ...
    .filter(Boolean) as SearchableSelectPlusOption[]
);
...
function toggleOption(opt: SearchableSelectPlusOption) {
...
function deselect(opt: SearchableSelectPlusOption) {
  const val = getOptionValue(opt);
  emit('update:modelValue', currentValues.value.filter((v) => String(v).toLowerCase() !== String(val).toLowerCase()));
}
